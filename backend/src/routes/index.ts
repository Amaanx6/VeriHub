...
function cleanEntity(value: string, type: string): string | null {
  switch (type) {
    case 'phone':
    ...
    case 'bank_account':
      return value.match(/\d{8,18}/)?.[0] || null;
    default:
      return value.trim();
  }
}

function getRecencyScore(reports: ReportData[]): number {
...
function getEntityTypeMultiplier(entityType: string): number {
...
function calculateRiskScore(reports: ReportData[], entityType: string): number {
...
function getSeverityBreakdown(reports: ReportData[]): Record<string, number> {
...
function getCategoryBreakdown(reports: ReportData[]): Record<string, number> {
  return reports.reduce((acc, report) => {
...
function generateInsights(reports: ReportData[], entity: ExtractedEntity): string[] {
...
router.get('/source-profile', async (req: Request, res: Response) => {
  try {
    // Properly handle query parameter types
    const getRiskThreshold = (): number => {
      const param = req.query.risk_threshold;
      if (Array.isArray(param)) {
        return parseInt(param[0] as string) || 4;
      }
      return parseInt(param as string) || 4;
    };
    ...
    const getLimit = (): number => {
      ...
      if (Array.isArray(param)) {
        return parseInt(param[0] as string) || 50;
      }
      return parseInt(param as string) || 50;
    };
    ...
    const allReports: ReportData[] = await prisma.report.findMany({
      select: {
        id: true,
        description: true,
        flaggedContent: true,
    ...
    const entityMap = new Map<string, EntityProfile>();
    
    // Process each report to extract entities
    allReports.forEach((report: ReportData) => {
      const textFields = [
        report.description,
      ...
      const combinedText = textFields.filter((field): field is string => field !== null).join(' ');
...
router.get('/source-profile/:reportId', async (req: Request<{ reportId: string }>, res: Response) => {
  try {
    const { reportId } = req.params;

    const report: ReportData | null = await prisma.report.findUnique({
      where: { id: reportId },
      select: {
        id: true,
        description: true,
    ...
    const profiledSources = await Promise.all(
      entities.map(async (entity) => {
        // Search for this entity in other reports
        const searchConditions = [
          { description: { contains: entity.value, mode: 'insensitive' as const } },
          { flaggedContent: { contains: entity.value, mode: 'insensitive' as const } },
          { reason: { contains: entity.value, mode: 'insensitive' as const } },
          { correction: { contains: entity.value, mode: 'insensitive' as const } },
          { additionalContext: { contains: entity.value, mode: 'insensitive' as const } }
        ];

        const linkedReports: ReportData[] = await prisma.report.findMany({
          where: {
            AND: [
              { OR: searchConditions },
              { id: { not: reportId } }
    ...
    return res.status(200).json({
      ...
      summary: {
        ...
        highest_risk_score: validSources[0]?.risk_score || 0,
...
router.get('/pattern-detect', async (req: Request, res: Response) => {
  try {
    const {
    ...
    const threshold = parseFloat(Array.isArray(similarity_threshold) 
      ? similarity_threshold[0] as string 
      : similarity_threshold as string
    );

    const minSize = parseInt(Array.isArray(min_cluster_size)
      ? min_cluster_size[0] as string
      : min_cluster_size as string
    );
...
function createClusters(reports: any[], threshold: number) {
  const clusters: { reports: any[], avgSimilarity: number }[] = [];
  const used = new Set<string>();

  for (let i = 0; i < reports.length; i++) {
    if (used.has(reports[i].id)) continue;
    ...
    for (let j = i + 1; j < reports.length; j++) {
      if (used.has(reports[j].id)) continue;
...
function extractDomain(url: string): string {
  try {
    if (!url || url === '') return 'direct-report';
    if (url.includes('whatsapp') || url.includes('telegram')) return 'messaging-app';
    if (url.includes('facebook') || url.includes('instagram')) return 'social-media';
...
function calculateClusterRisk(reports: any[], domainCount: number): string {
  ...
  if (reportCount >= 10) riskScore += 3;
  else if (reportCount >= 5) riskScore += 2;
  else if (reportCount >= 3) riskScore += 1;

  // Total mentions across reports
  if (totalMentions >= 20) riskScore += 2;
  else if (totalMentions >= 10) riskScore += 1;

  // Severity distribution
  if (criticalCount >= 2) riskScore += 3;
  else if (highCount >= 3) riskScore += 2;
  else if (criticalCount >= 1) riskScore += 1;

  // Domain spread (indicates coordination)
  if (domainCount >= 5) riskScore += 3;
  else if (domainCount >= 3) riskScore += 2;
  else if (domainCount >= 2) riskScore += 1;
...
function generateMainClaim(reports: any[]): string {
...
function generateClusterInsights(reports: any[], domains: string[]): string[] {
  const insights: string[] = [];
  const reportCount = reports.length;
  const totalMentions = reports.reduce((sum, r) => sum + r.reportCount, 0);
